      ...
      <form @submit.prevent>
        ...
        <div class="field grid-50-50">
          <div class="form-group">
            ...
            <input v-model="userData.password" name="oldPassword" type="password" class="form-control"/>
          </div>
          <div class="form-group">
            <label for="newPassword">New Password</label>
            <input v-model="userData.password" name="newPassword" type="password" class="form-control"/>
          </div>
        </div>

        <div class="form-group">
          <button @click="updateUserData" class="btn btn-primary btn-block">
            <span>Update data</span>
          </button>
        </div>

        <div class="form-group">
          <button @click="disableUser" class="btn btn-primary btn-block">
...
import {getToken, handleLogOut} from "@/utils/utils";
...
export default {
  ...
  methods: {
    ...
    updateUserData() {
      axios.post("http://localhost:8080/edit", this.userData,
          {headers: {'Authorization': 'Bearer ' + getToken()}}).then((resp) => {
        this.userData = resp.data;
        this.$router.go("/info");
      }).catch(error => {
    ...
    disableUser() {
      axios.post("http://localhost:8080/disable", {},
